...
def Format_Network_New_Inbound(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Format_Network_New_Inbound() called')
    
    template = """| tstats `summariesonly` values(All_Traffic.action) as action,values(All_Traffic.src_port) as src_port,count from datamodel=Network_Traffic.All_Traffic where ( All_Traffic.dest=\"{0}\" ) by _time, All_Traffic.src, All_Traffic.dest 
...
| table src, dest, today, count | sort - count"""
    ...
    phantom.format(container=container, template=template, parameters=parameters, name="Format_Network_New_Inbound")

    Check_Network_New_Inbound(container=container)
...
"""
def Check_Network_New_Inbound(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Check_Network_New_Inbound() called')
        
    #phantom.debug('Action: {0} {1}'.format(action['name'], ('SUCCEEDED' if success else 'FAILED')))
    
    # collect data for 'Check_Network_New_Inbound' call
    formatted_data_1 = phantom.get_format_data(name='Format_Network_New_Inbound')

    parameters = []
    
    # build parameters list for 'Check_Network_New_Inbound' call
    parameters.append({
    ...
    phantom.act(action="run query", parameters=parameters, assets=['splunk es - ot sec'], callback=filter_1, name="Check_Network_New_Inbound")
...
def filter_1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    matched_artifacts_1, matched_results_1 = phantom.condition(
        ...
        conditions=[
            ["Check_Network_New_Inbound:action_result.summary.total_events", ">", 0],
        ],
        name="filter_1:condition_1")

    # call connected blocks if filtered artifacts or results
    if matched_artifacts_1 or matched_results_1:
        Format_Pin_Network_New_Inbound(action=action, success=success, container=container, results=results, handle=handle, custom_function=custom_function, filtered_artifacts=matched_artifacts_1, filtered_results=matched_results_1)
...
def Format_Pin_Network_New_Inbound(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Format_Pin_Network_New_Inbound() called')
    
    template = """Total {0} Records Detected for Review"""

    # parameter list for template variable replacement
    parameters = [
        "filtered-data:filter_1:condition_1:Check_Network_New_Inbound:action_result.summary.total_events",
    ]

    phantom.format(container=container, template=template, parameters=parameters, name="Format_Pin_Network_New_Inbound")
...
def Pin_Check_Network_New_Inbound(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    formatted_data_1 = phantom.get_format_data(name='Format_Pin_Network_New_Inbound')

    phantom.pin(container=container, data=formatted_data_1, message="Check Network New Inbound Traffic", name="Check Network New Inbound Traffic")
...
def Add_Note_Format(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    template = """**GUIDE** : Check Network New Inbound Traffic detected for the asset.  The listed sessions are a new session to hosts as compared to yesterday.
...
%%"""
    ...
    # parameter list for template variable replacement
    parameters = [
        "Check_Network_New_Inbound:action_result.data.*.src",
        "Check_Network_New_Inbound:action_result.data.*.dest",
        "Check_Network_New_Inbound:action_result.data.*.today",
        "Check_Network_New_Inbound:action_result.data.*.count",
    ]
...
def Search_OT_Asset(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    phantom.act(action="run query", parameters=parameters, assets=['splunk es - ot sec'], callback=Format_Network_New_Inbound, name="Search_OT_Asset")
