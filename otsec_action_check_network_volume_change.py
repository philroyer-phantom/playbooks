...
def Format_Network_Volume_Change(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Format_Network_Volume_Change() called')
...
| rename dvc as \"Asset\", dvc_asset_type as \"Asset Type\", tot_bytes_change_pct as \"Traffic Amount Change\", tot_session_change_pct as \"Session Amount Change\", tot_duration_change_pct as \"Duration Change\",  dvc_asset_status as \"Asset Status\", dvc_asset_system as \"Asset System\", tot_mb_avg as \"Traffic Amount (MB)\",	tot_session_avg as \"Session Amount\", tot_duration_avg as \"Duration\", site_id as \"Facility\""""
    ...
    phantom.format(container=container, template=template, parameters=parameters, name="Format_Network_Volume_Change")

    Check_Network_Volume_Change(container=container)
...
"""
def Check_Network_Volume_Change(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Check_Network_Volume_Change() called')
        
    #phantom.debug('Action: {0} {1}'.format(action['name'], ('SUCCEEDED' if success else 'FAILED')))
    
    # collect data for 'Check_Network_Volume_Change' call
    formatted_data_1 = phantom.get_format_data(name='Format_Network_Volume_Change')

    parameters = []
    
    # build parameters list for 'Check_Network_Volume_Change' call
    parameters.append({
    ...
    phantom.act(action="run query", parameters=parameters, assets=['splunk es - ot sec'], callback=filter_1, name="Check_Network_Volume_Change")
...
def filter_1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    matched_artifacts_1, matched_results_1 = phantom.condition(
        ...
        conditions=[
            ["Check_Network_Volume_Change:action_result.summary.total_events", ">", 0],
        ],
        name="filter_1:condition_1")

    # call connected blocks if filtered artifacts or results
    if matched_artifacts_1 or matched_results_1:
        Format_Pin_Network_Volume_Change(action=action, success=success, container=container, results=results, handle=handle, custom_function=custom_function, filtered_artifacts=matched_artifacts_1, filtered_results=matched_results_1)
...
def Format_Pin_Network_Volume_Change(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Format_Pin_Network_Volume_Change() called')
    
    template = """Total {0} Records Detected for Review"""

    # parameter list for template variable replacement
    parameters = [
        "filtered-data:filter_1:condition_1:Check_Network_Volume_Change:action_result.summary.total_events",
    ]

    phantom.format(container=container, template=template, parameters=parameters, name="Format_Pin_Network_Volume_Change")

    Pin_Check_Network_Volume_Change(container=container)

    return

def Pin_Check_Network_Volume_Change(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    phantom.debug('Pin_Check_Network_Volume_Change() called')

    formatted_data_1 = phantom.get_format_data(name='Format_Pin_Network_Volume_Change')

    phantom.pin(container=container, data=formatted_data_1, message="Check Network Volume Change", name="Check Network Volume Change")
...
def Add_Note_Format(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    template = """|src|dest|dates|bytes|bytes_min|bytes_max|bytes_diff|
|--|--|--|--|--|--|--|
%%
|{0}|{1}|{2}|{3}|{4}|{5}|{6}|
%%"""
    ...
    # parameter list for template variable replacement
    parameters = [
        "Check_Network_Volume_Change:action_result.data.*.src",
        "Check_Network_Volume_Change:action_result.data.*.dest",
        "Check_Network_Volume_Change:action_result.data.*.dates",
        "Check_Network_Volume_Change:action_result.data.*.bytes",
        "Check_Network_Volume_Change:action_result.data.*.bytes_min",
        "Check_Network_Volume_Change:action_result.data.*.bytes_max",
        "Check_Network_Volume_Change:action_result.data.*.bytes_diff",
    ]
...
def add_note_3(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    note_title = "Check Network Volume Change"
...
def Search_OT_Asset(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, **kwargs):
    ...
    phantom.act(action="run query", parameters=parameters, assets=['splunk es - ot sec'], callback=Format_Network_Volume_Change, name="Search_OT_Asset")
